...
#
# ==============================================================================
# ssh-policy.sh — SSH Hardening + optional Passwordless sudo (Ubuntu/Debian)
#
# Speichern:        nano ssh-policy.sh
# Ausführbar:       chmod +x ssh-policy.sh
# Starten (sudo):   sudo ./ssh-policy.sh
#
# Was es macht:
...
echo "   2. Verbinde dich mit dem Server per SSH-Key:"
if [[ "${SET_CUSTOM_SSH_PORT}" == "yes" ]]; then
  echo "      ssh -p ${CUSTOM_SSH_PORT} ${SUDO_USER:-$USER}@$(hostname -I | awk '{print $1}')"
else
  echo "      ssh -i ~/.ssh/id_ed25519_${SUDO_USER:-$USER} ${SUDO_USER:-$USER}@$(hostname -I | awk '{print $1}')"
fi
